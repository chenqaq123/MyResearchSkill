...
Before writing the Introduction, formulate the paper's central claim in one sentence.
The Introduction should not be a flat sequence of background, prior work, gap, method, and results.
It should build a narrative that makes the central claim feel necessary, non-obvious, and verifiable.

Use this claim-thread:

1. Why does the task or setting matter?
2. What difficulty prevents existing approaches from solving it well?
3. What central claim does this paper make?
4. What method idea makes the claim plausible?
5. What evidence makes the claim credible?
6. What impact or takeaway should readers remember?

If a paragraph does not make the central claim easier to understand, believe, or evaluate, revise it, move it, or remove it.

## User-Preferred Introduction Model

When the paper context allows, use this reader-facing sequence:

1. Big background: the broad research area and why it matters.
2. Small background: the specific task, setting, or technical subproblem.
3. Related work: the most relevant prior solution lines, summarized by mechanism rather than citation list.
4. Research gap and difficulty: state not only what is missing, but why the gap is technically difficult.
5. What we propose: name the proposed method, system, benchmark, or framework.
6. Method overview: briefly explain how the proposal addresses the stated difficulty.
7. Experimental results and important theory: summarize the strongest empirical evidence, theorem, or formal guarantee when available.
8. Contributions and impact: separate the meaning of the work into problem/idea, method, experiment, and broader significance when appropriate.

The difficulty statement is mandatory when writing Introduction drafts.
Do not describe a gap only as "prior methods cannot do X"; explain the obstacle, constraint, or mechanism that makes X hard.
If the difficulty, result, theorem, or impact is not yet concrete, insert a TODO at the exact location.
...
% Big background
% Small background / task setting
% Related work and prior solution lines
% Research gap and technical difficulty
% What we propose
% Method overview: how it solves the difficulty
% Experimental results and important theorem if applicable
